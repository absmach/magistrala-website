...
export const solutions: Solution[] = [
  ...
  // ── Cold Storage ──────────────────────────────────────────────────────
  {
    ...
    whyMagistrala:
      "Your existing temperature sensors, CO2 monitors, and door contacts probably already speak MQTT or HTTP, so there's nothing to replace in the field. Every SenML record is persisted with its original device timestamp as it arrives — the same raw reading that fires the alarm is what the Monthly Compliance Report is built from, with nothing stripped or transformed in between. One Cold Room Status template covers any number of customer storage zones by device tag, so a facility managing multiple consignment rooms doesn't need separate dashboard configurations for each. Daily temperature logs and monthly HACCP compliance reports run and deliver automatically — no manual data assembly before the auditor arrives.",
