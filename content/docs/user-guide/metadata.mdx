...
### UI-Managed User Preferences

When a user first logs in, the Magistrala UI writes the following keys into `metadata` as defaults:

| Key | Default value | Description |
|-----|---------------|-------------|
| `theme` | `"default"` | Active UI theme |
| `language` | `"en"` | UI display language |
| `subscribed` | `false` | Email notification subscription |

These keys will be **set by the UI on first login**, so any values pre-set via the API or CLI for these keys will be overwritten at that point.

Example of a user's metadata after first login:
