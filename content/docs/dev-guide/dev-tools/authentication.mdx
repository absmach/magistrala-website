...
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| **`MG_GOOGLE_CLIENT_ID`**      | Google OAuth 2.0 client ID obtained from Google Cloud Console.                                                      |
| **`MG_GOOGLE_CLIENT_SECRET`**  | Google OAuth 2.0 client secret associated with the client ID.                                                       |
| **`MG_GOOGLE_REDIRECT_URL`**   | OAuth 2.0 callback endpoint for the Magistrala backend. Must match the authorized redirect URI in your Google project. |
| **`MG_GOOGLE_STATE`**          | Random unique string to protect against CSRF attacks during OAuth flow.                                             |
| **`MG_OAUTH_UI_REDIRECT_URL`** | URL where the UI receives tokens after successful authentication (e.g., NextAuth `/api/auth/token`).                |
| **`MG_OAUTH_UI_ERROR_URL`**    | Fallback URL for failed authentications (e.g., login page).                                                         |
...
8. The user is redirected to **`MG_OAUTH_UI_REDIRECT_URL`**.
