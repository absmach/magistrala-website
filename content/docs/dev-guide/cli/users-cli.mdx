...
- **`metadata`** — publicly readable. The UI uses this to store user preferences (theme, language, etc.). Follows the same typed format as other entities.
- **`private_metadata`** — never returned in API responses to the user. Intended for server-side or admin-managed data that should not be client-visible.
