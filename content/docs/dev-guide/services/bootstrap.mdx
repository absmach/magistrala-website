...
The response body is the created config. The `id` is server-generated. `status` is always `disabled` on creation regardless of what is sent. `name` must be unique within a domain — attempting to create a second config with the same name in the same domain returns `409 Conflict`.
